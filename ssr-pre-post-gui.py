        ...
        for router in selected_routers:
            ...
            for key in enabled_checks:
                ...
                try:
                    ...
                    q.put(("progress", router, label, "err", f"{type(e)._name_}: {e}"))
                done += 1
...
class DetailDialog(tk.Toplevel):
    def _init_(self, parent, change, debug=False):
        super()._init_(parent)
...
class RouterTab(ttk.Frame):
    def _init_(self, parent, rows, debug=False):
        super()._init_(parent)
        self._all   = rows
        self._debug = debug
        self._build()
...
class DiffViewer(tk.Toplevel):
    def _init_(self, parent, diff_data, debug=False):
        super()._init_(parent)
